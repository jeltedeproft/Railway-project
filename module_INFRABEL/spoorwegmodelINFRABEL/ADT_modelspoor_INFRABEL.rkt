...
(require graph)
(require "ADT_Train_INFRABEL.rkt")
(require "ADT_Arc_INFRABEL.rkt")
(require "ADT_Detection_Block_INFRABEL.rkt")
(require "ADT_Node_INFRABEL.rkt")
(require "ADT_Switch_INFRABEL.rkt")

(provide make-model)
(provide train-identifier)
(provide node-identifier)
(provide switch-identifier)
(provide detection-block-identifier)
...
(define (make-model filename)
  (let* ((current-graph (weighted-graph/undirected '()))
    ...
    (define trains (map (lambda (train) (make-ADT-train-mivb train))
                       trains-list))
    
    ;2) create the node ADT's
    (define nodes (map (lambda (node) (make-ADT-node-mivb node))
                      nodes-list))
    
    ;3) create the switch ADT's
    (define switches (map (lambda (switch) (make-ADT-switch-mivb switch))
                         switches-list))
    
    ;4) create the detection-block ADT's
    (define detection-blocks (map (lambda (detection-block) (make-ADT-detection-block-mivb detection-block))
                                 detection-blocks-list))
    
    ;5) create the arc ADT's
    (define arcs (map (lambda (arc) (make-ADT-arc-mivb arc))
                     arcs-list))
    ...
    (map (lambda (switch) (add-vertex! current-graph switch))
        switches)
    ...
    ;the trains spawn on certain detection blocks, so we put the lights of those detection blocks to red immediately
    (for-each (lambda (train)
                (((lookup-ADT detection-block-identifier (symbol->string (((lookup-ADT train-identifier ((train 'get-name))) 'get-last-seen-detection-block)))) 'set-state-left-light) 'red)
                (((lookup-ADT detection-block-identifier (symbol->string (((lookup-ADT train-identifier ((train 'get-name))) 'get-last-seen-detection-block)))) 'set-state-right-light) 'red))
             trains)
    
    ;now we set the lights of the detection blocks that the trains started on to red
    (for-each (lambda (train)
                (let ((det-block (lookup-ADT detection-block-identifier ((train 'get-last-seen-detection-block)))))
                  ((det-block 'set-state-left-light) 'red)
                  ((det-block 'set-state-right-light) 'red)))
             trains)
